...
if os.path.exists(OPATH):
    try:
        ...
        def get_hemisphere(surface_number, hemi):
            thisname_mgh = "{output}{bids_id}_space-fsaverage5_desc-{hemi}h_MPC-{surface_number:d}.mgh".format(output=OPATH, bids_id=bids_id, hemi=hemi, surface_number=surface_number+1)
        ...
        pathToParc = pipeline_dir + "/parcellations/"
        ...
        if np.isnan(np.sum(MPC)):
            ...
            np.savetxt("{output}/{bids_id}_space-fsaverage5_atlas-{parc_str}_desc-MPC.txt".format(output=OPATH, parc_str=parc_str, bids_id=bids_id), MPC, fmt='%.6f')
            np.savetxt("{output}/{bids_id}_space-fsaverage5_atlas-{parc_str}_desc-intensity_profiles.txt".format(output=OPATH, parc_str=parc_str, bids_id=bids_id), I, fmt='%.12f')
            print("")
            print("-------------------------------------")
